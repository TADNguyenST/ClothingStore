...
public class InventoryDAO {
    ...
    public List<Product> getAllProducts() throws SQLException {
        ...
        try ( Connection conn = new DBContext().getConnection();  PreparedStatement ps = conn.prepareStatement(sql);  ResultSet rs = ps.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                product.setBrandId(rs.getObject("brand_id", Long.class));
        ...
        try ( Connection conn = new DBContext().getConnection();  PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try ( ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    // === BỔ SUNG ĐẦY ĐỦ CÁC TRƯỜNG BỊ THIẾU ===
                    product.setProductId(rs.getLong("product_id"));
                    product.setName(rs.getString("name"));
                    product.setDescription(rs.getString("description"));
                    product.setPrice(rs.getBigDecimal("price"));
                    product.setCategoryId(rs.getObject("category_id", Long.class)); // Rất quan trọng
                    product.setBrandId(rs.getObject("brand_id", Long.class));       // Rất quan trọng
                    product.setMaterial(rs.getString("material"));
                    product.setStatus(rs.getString("status"));
    ...
    public void addStockMovement(StockMovement movement, Connection conn) throws SQLException {
        ...
        try ( PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setInt(3, movement.getQuantityChanged());
