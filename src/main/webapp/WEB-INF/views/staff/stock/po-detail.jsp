                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <form action="PurchaseOrder" method="post" id="poForm">
                            ...
                            <div class="mb-4 row">
                                <label class="col-sm-2 col-form-label"><b>Supplier</b></label>
                                <div class="col-sm-10">
                                    <c:choose>
                                        <%-- Nếu đơn hàng là bản nháp, hiển thị dropdown để chọn --%>
                                        <c:when test="${poData.get('status') == 'Draft'}">
                                        ...
                                        <c:otherwise>
                                            ...
                                            <p class="form-control-plaintext">${poData.get('supplierName')}</p>
                                            <input type="hidden" name="supplierId" value="${poData.get('supplierId')}">
                                        </c:otherwise>
                                    </c:choose>
                                </div>
                            </div>
                                ...
                                <table class="table table-bordered table-hover">
                                    ...
                                    <c:forEach var="item" items="${itemsInPO}">
                                        <tr>
                                            ...
                                            <td>
                                                <c:if test="${poData.get('status') == 'Draft'}">
                                                    <button type="button" class="btn btn-danger btn-sm" onclick="showConfirmationModal('Delete Product', 'Are you sure you want to delete this product from the order?', () => {
                                                                window.location.href = 'PurchaseOrder?action=deleteItem&podId=${item.get('podId')}&poId=${poData.get('poId')}'
                                                            })">
                            ...
                            <div class="action-buttons">
                                ...
                                <c:if test="${poData.get('status') == 'Draft'}">
                                    ...
                                    <button type="button" class="btn btn-danger" onclick="showConfirmationModal('Cancel Order', 'Are you sure you want to cancel this draft?', () => {
                                                window.location.href = 'PurchaseOrder?action=cancel&poId=${poData.get('poId')}'
                                            })">
                                        ...
                                        document.addEventListener('DOMContentLoaded', function () {
                                            ...
                                            function handleFormAction(action) {
                                                ...
                                                if (action === 'finalize') {
                                                    ...
                                                    if (!supplierId) {
                                                        showModal('Validation Error', 'Please select a supplier before completing the order.', null, true);
                                                        return;
                                                    }
                                                    if (itemRows.length === 0) {
                                                        showModal('Validation Error', 'Cannot complete the order because there are no products.', null, true);
                                                        return;
                                                    }
                                            ...
                                            poForm.addEventListener('click', (e) => {
                                                ...
                                                if (onclickAttr.includes("submitFormAction('saveDraft')")) {
                                                    handleFormAction('saveDraft');
                                                } else if (onclickAttr.includes("submitFormAction('finalize')")) {
                                                    handleFormAction('finalize');
                                                } else if (onclickAttr.includes('action=cancel')) {
                                                    const url = onclickAttr.match(/window\.location\.href = '(.+?)'/)[1];
                                                    showModal('Cancel Order', '<b>Any unsaved changes will be lost.</b><br>Are you sure you want to cancel this draft?', () => {
                                                        window.location.href = url;
                                                    });
                                                } else if (onclickAttr.includes('action=deleteItem')) {
                                                    const url = onclickAttr.match(/window\.location\.href = '(.+?)'/)[1];
                                                    showModal('Delete Product', '<b>Any unsaved changes will be lost.</b><br>Are you sure you want to delete this product?', () => {
                                                        window.location.href = url;
                                                    });
                                                }
                                            });
                                            ...
                                            document.addEventListener('click', function (event) {
                                                ...
                                                if (!link || poForm.contains(link) || !hasUnsavedChanges || link.getAttribute('href').startsWith('#') || link.hasAttribute('data-bs-toggle')) {
                                                    return;
                                                }
                                                event.preventDefault();
                                                showModal('Unsaved Changes', 'You have unsaved changes that will be lost. Are you sure you want to leave this page?', () => {
                                                    hasUnsavedChanges = false;
