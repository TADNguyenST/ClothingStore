...
<%-- Lấy dữ liệu từ request scope để sử dụng --%>
...
<c:set var="productReportData" value="${report.productReportData}" />
<c:set var="ordersReportData" value="${report.ordersReportData}" />
<c:set var="productKpis" value="${report.productKpis}" />
<c:set var="systemKpis" value="${report.systemKpis}" />
<c:set var="systemChartData" value="${report.systemRevenueChartData}" />
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function renderProductChart(data, type) {
                    ...
                    productChartInstance = new Chart(ctx, {
                        ...
                        data: {
                            labels: labels,
                            datasets: [{label: chartLabel, data: values, backgroundColor: '#0d6efd'}]
                        },
                ...
                function renderFullProductTable(data) {
                    ...
                    data.forEach(item => {
                        rowsHtml += '<tr>' +
                                ...
                                '<td class="text-end">' + (item.totalQuantitySold || 0).toLocaleString() + '</td>' +
                                '<td class="text-end">' + currencyFormatter.format(item.totalRevenue || 0) + '</td>' +
                                '</tr>';
                    });
                ...
                function renderOrdersTable(data) {
                    ...
                    data.forEach(item => {
                        rowsHtml += '<tr>' +
                                ...
                                '<td>' + dateFormatter.format(new Date(item.orderDate)) + '</td>' +
                                '<td class="text-center">' + item.itemCount + '</td>' +
                                '<td class="text-end">' + currencyFormatter.format(item.totalPrice) + '</td>' +
                                '</tr>';
                    });
                    tableBody.innerHTML = rowsHtml;
                }
                ...
                function updateSortableHeaders() {
                    document.querySelectorAll('.sortable .fa').forEach(icon => {
                    ...
                    if (activeProductSort)
                        activeProductSort.querySelector('.fa').className = `fa fa-sort-${productSortOrder.toLowerCase()}`;

                    let activeOrderSort = document.querySelector(`#order-table-header [data-sortby="${orderSortBy}"]`);
                    if (activeOrderSort)
                        activeOrderSort.querySelector('.fa').className = `fa fa-sort-${orderSortOrder.toLowerCase()}`;
                }
