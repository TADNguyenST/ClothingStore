...
<body>
    ...
    <c:set var="currentAction" value="categoryForm" scope="request"/>
                        ...
                        <div class="box-body">
                            ...
                            <c:choose>
                                <c:when test="${not empty category}">
                                    <form action="${pageContext.request.contextPath}/CategoryManager" method="post" onsubmit="return validateForm()" class="mt-3">
                                        <input type="hidden" name="action" value="update">
                                        <input type="hidden" name="id" value="${category.categoryId}">
                                        <div class="mb-3">
                                            <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                                            <input type="text" id="name" name="name" class="form-control" value="${category.name}" required maxlength="100" placeholder="Enter category name">
                                            <div class="invalid-feedback">Please enter a valid category name (max 100 characters).</div>
                                        </div>
                                        <div class="mb-3">
                                            <label for="description" class="form-label">Description</label>
                                            <textarea id="description" name="description" class="form-control" rows="4" placeholder="Enter category description">${category.description}</textarea>
                                        </div>
                                        <div class="mb-3">
                                            ...
                                            <select id="parentCategoryId" name="parentCategoryId" class="form-select">
                                                ...
                                                <c:forEach var="parent" items="${categories}">
                                                    <c:if test="${empty parent.parentCategoryId && parent.categoryId != category.categoryId}">
                                                        <option value="${parent.categoryId}" ${parent.categoryId == category.parentCategoryId ? 'selected' : ''}>${parent.name}</option>
                                                    </c:if>
                                                </c:forEach>
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            ...
                                            <select id="isActive" name="isActive" class="form-select">
                                                ...
                                                <option value="false" ${!category.active ? 'selected' : ''}>Discontinued</option>
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            <button type="submit" class="btn btn-primary"><i class="bi bi-save"></i> Update Category</button>
                                            <a href="${pageContext.request.contextPath}/CategoryManager?action=list" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Cancel</a>
                                        </div>
                                    </form>
                                </c:when>
                                <c:otherwise>
                                    ...
                                    <div class="mb-3">
                                        <a href="${pageContext.request.contextPath}/CategoryManager?action=list" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Back to List</a>
                                    </div>
                                </c:otherwise>
                            </c:choose>
                        </div>
